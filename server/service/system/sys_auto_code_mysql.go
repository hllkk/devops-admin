...
import (
	...
	"github.com/hllkk/devops-admin/server/model/system/response"
)
...
func (s *autoCodeMysql) GetDB(businessDB string) (data []response.Db, err error) {
	...
	if businessDB == "" {
		err = global.OPS_DB.Raw(sql).Scan(&entities).Error
	} else {
		err = global.OPS_DBList[businessDB].Raw(sql).Scan(&entities).Error
	}
	return entities, err
}
...
func (s *autoCodeMysql) GetTables(businessDB string, dbName string) (data []response.Table, err error) {
	...
	if businessDB == "" {
		err = global.OPS_DB.Raw(sql, dbName).Scan(&entities).Error
	} else {
		err = global.OPS_DBList[businessDB].Raw(sql, dbName).Scan(&entities).Error
	}
...
ORDER BY 
    c.ORDINAL_POSITION;`
	if businessDB == "" {
		err = global.OPS_DB.Raw(sql, tableName, dbName).Scan(&entities).Error
	} else {
		err = global.OPS_DBList[businessDB].Raw(sql, tableName, dbName).Scan(&entities).Error
	}
