...
func (*Qiniu) UploadFile(ctx context.Context, file *multipart.FileHeader) (string, string, error) {
	putPolicy := storage.PutPolicy{Scope: global.OPS_CONFIG.Qiniu.Bucket}
	mac := qbox.NewMac(global.OPS_CONFIG.Qiniu.AccessKey, global.OPS_CONFIG.Qiniu.SecretKey)
	...
	return global.OPS_CONFIG.Qiniu.ImgPath + "/" + ret.Key, ret.Key, nil
}
...
func (*Qiniu) DeleteFile(ctx context.Context, key string) error {
	...
	if err := bucketManager.Delete(global.OPS_CONFIG.Qiniu.Bucket, key); err != nil {
...
func (*Qiniu) Exists(ctx context.Context, key string) (bool, error) {
	bucketManager := newBucketManager()
	if _, err := bucketManager.Stat(global.OPS_CONFIG.Qiniu.Bucket, key); err != nil {
		msg := err.Error()
		// 七牛 612 表示资源不存在
...
func (*Qiniu) DeleteFiles(ctx context.Context, keys []string) ([]DeleteFailure, error) {
	...
	bucket := global.OPS_CONFIG.Qiniu.Bucket
...
func (*Qiniu) ListFiles(ctx context.Context, prefix, cursor string, limit int) ([]FileInfo, string, bool, error) {
	...
	bucketManager := newBucketManager()
	entries, _, nextMarker, hasNext, err := bucketManager.ListFiles(global.OPS_CONFIG.Qiniu.Bucket, prefix, "", cursor, limit)
	if err != nil {
...
func qiniuConfig() *storage.Config {
	cfg := storage.Config{
		UseHTTPS:      global.OPS_CONFIG.Qiniu.UseHTTPS,
		UseCdnDomains: global.OPS_CONFIG.Qiniu.UseCdnDomains,
	}
	switch global.OPS_CONFIG.Qiniu.Zone { // 根据配置文件进行初始化空间对应的机房
	case "ZoneHuadong":
