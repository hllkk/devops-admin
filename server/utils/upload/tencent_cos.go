...
func (*TencentCOS) UploadFile(ctx context.Context, file *multipart.FileHeader) (string, string, error) {
	...
	_, err := client.Object.Put(context.Background(), global.OPS_CONFIG.TencentCOS.PathPrefix+"/"+fileKey, f, nil)
	if err != nil {
	...
	return global.OPS_CONFIG.TencentCOS.BaseURL + "/" + global.OPS_CONFIG.TencentCOS.PathPrefix + "/" + fileKey, fileKey, nil
}

// DeleteFile delete file form COS
func (*TencentCOS) DeleteFile(ctx context.Context, key string) error {
	client := NewClient()
	name := global.OPS_CONFIG.TencentCOS.PathPrefix + "/" + key
...
func (*TencentCOS) DeleteFiles(ctx context.Context, keys []string) (failed []DeleteFailure, err error) {
	...
	for _, k := range keys {
		objects = append(objects, cos.Object{Key: global.OPS_CONFIG.TencentCOS.PathPrefix + "/" + k})
	}
...
func NewClient() *cos.Client {
	urlStr, _ := url.Parse("https://" + global.OPS_CONFIG.TencentCOS.Bucket + ".cos." + global.OPS_CONFIG.TencentCOS.Region + ".myqcloud.com")
	baseURL := &cos.BaseURL{BucketURL: urlStr}
	client := cos.NewClient(baseURL, &http.Client{
		Transport: &cos.AuthorizationTransport{
			SecretID:  global.OPS_CONFIG.TencentCOS.SecretID,
			SecretKey: global.OPS_CONFIG.TencentCOS.SecretKey,
		},
	})
	return client
}
