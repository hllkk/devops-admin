...
const passwordRules: Record<PasswordRuleKey, App.Global.FormRule> = {
  oldPassword: createRequiredRule($t('page.userCenter.password.form.oldPassword')),
  confirmPassword: createRequiredRule($t('page.userCenter.password.form.confirmPassword')),
  newPassword: createRequiredRule($t('page.userCenter.password.form.newPassword'))
};
...
async function updateProfile() {
  ...
  if (!error) {
    window.$message?.success($t('page.userCenter.profile.updateSuccess'));
    // 更新本地用户信息
    if (userInfo.user) {
...
async function updatePassword() {
  ...
  if (passwordModel.newPassword !== passwordModel.confirmPassword) {
    window.$message?.error($t('page.userCenter.password.inconsistent'));
    return;
  }
  startBtnLoading();
  ...
  if (!error) {
    window.$message?.success($t('page.userCenter.password.modifySuccess'));
    // 清空表单
...
<template>
  <div class="flex gap-16px">
    ...
    <NCard :title="$t('page.userCenter.personalInfo')" class="w-360px shadow-sm">
      <div class="flex-x-center flex-wrap gap-24px">
        ...
        <NDescriptions :column="1" label-placement="left" label-width="120px">
          <NDescriptionsItem :label="$t('page.userCenter.phone')">
    ...
    <!-- 基本资料卡片 -->
    <NCard :title="$t('page.userCenter.baseProfile')" class="w-full overflow-x-auto shadow-sm">
      <NTabs type="line" animated class="h-full" s>
        <NTabPane name="userInfo" :tab="$t('page.userCenter.tab.profile')">
          <NForm
            ref="profileFormRef"
            :model="profileModel"
            :rules="profileRules"
          ...
          >
            ...
            <NFormItem :label="$t('page.userCenter.profile.sex')" path="sex">
              <NRadioGroup v-model:value="profileModel.sex">
                <NRadio value="0">{{ $t('page.userCenter.profile.male') }}</NRadio>
                <NRadio value="1">{{ $t('page.userCenter.profile.female') }}</NRadio>
              </NRadioGroup>
            </NFormItem>
        ...
        <NTabPane name="updatePwd" :tab="$t('page.userCenter.tab.updatePwd')">
          <NForm
            ref="passwordFormRef"
            :model="passwordModel"
            :rules="passwordRules"
          ...
          >
            <NFormItem :label="$t('page.userCenter.password.oldPassword')" path="oldPassword">
              <NInput
                ...
                type="password"
                :placeholder="$t('page.userCenter.password.placeholder.oldPassword')"
            ...
            <NFormItem :label="$t('page.userCenter.password.newPassword')" path="newPassword">
              <NInput
                ...
                type="password"
                :placeholder="$t('page.userCenter.password.placeholder.newPassword')"
                show-password-on="click"
              />
            </NFormItem>
            <NFormItem :label="$t('page.userCenter.password.confirmPassword')" path="confirmPassword">
              <NInput
                ...
                type="password"
                :placeholder="$t('page.userCenter.password.placeholder.confirmPassword')"
                show-password-on="click"
              />
            </NFormItem>
            <NFormItem class="flex items-center justify-end">
        ...
        <NTabPane name="social" :tab="$t('page.userCenter.tab.social')">
          <SocialCard />
        </NTabPane>
        <NTabPane name="online" :tab="$t('page.userCenter.tab.online')">
          <div class="h-full">
            <OnlineTable />
          </div>
        </NTabPane>
