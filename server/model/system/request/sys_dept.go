...
// Int64String 兼容前端 IdType(string|number)与空串的 int64 字段。
// 部门 parentId 在顶层新增时前端 createDefaultModel 传空串 "",普通 int64 字段的 JSON 绑定会失败,
// 故实现 UnmarshalJSON 容忍 "" / null / 数字 / "字符串数字",统一解析为 int64(空/null→0=顶层)。
...
// deptId/parentId 用 Int64String 兼容前端空串与 IdType 混合;leader 用 int64(前端 null→0)。
type DeptOperateParams struct {
	DeptId       Int64String `json:"deptId"`       // 部门ID(新增时为空)
	ParentId     Int64String `json:"parentId"`     // 父部门ID(空/0=顶层)
