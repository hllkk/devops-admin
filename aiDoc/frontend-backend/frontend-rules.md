...
| store/modules | `store/modules/{dict,notice,system}/`（admin 业务 store） | `store/modules/<m>/index.ts`（有全局状态才建） |
| typings/api | `<area>.api.d.ts`（如 `system.api.d.ts`） | `typings/api/<m>.api.d.ts` |
| locales | `route.admin_*` / `page.<area>.*` | `route.<m>_*` / `page.<m>.*`（zh-cn / en-us / `app.d.ts` schema 三处同步） |
