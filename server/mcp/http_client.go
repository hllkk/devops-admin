...
func ResolveMCPServiceURL() string {
	baseURL := strings.TrimSpace(global.OPS_CONFIG.MCP.BaseURL)
	if baseURL != "" {
		return strings.TrimRight(baseURL, "/")
	}

	addr := global.OPS_CONFIG.MCP.Addr
	if addr <= 0 {
		addr = 8889
	}

	path := strings.TrimSpace(global.OPS_CONFIG.MCP.Path)
	if path == "" {
		path = "/mcp"
	}
...
func upstreamBaseURL() string {
	baseURL := strings.TrimSpace(global.OPS_CONFIG.MCP.UpstreamBaseURL)
	if baseURL != "" {
		return strings.TrimRight(baseURL, "/")
	}
...
func requestTimeout() time.Duration {
	timeout := global.OPS_CONFIG.MCP.RequestTimeout
	if timeout <= 0 {
		timeout = 15
	}
