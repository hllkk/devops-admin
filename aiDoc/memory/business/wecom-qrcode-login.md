...
- 自动建号:首次扫码按 `SysGeneralConfig.DefaultRoleId` 创建 `sys_user` + `sys_social(wecom)` 关联
...
- 建号策略:**自动建号 + 默认角色**(默认角色取自 `SysGeneralConfig.DefaultRoleId`——常规配置通用项,供企微/LDAP/后续注册复用;为 0 则建号失败)。用户名 `wecom_<userid>`,密码随机不可用,`PasswordUpdatedAt=now` 防过期判定
...
- model:`sys_social` 加 `Mobile`、`sys_general_config` 加 `DefaultRoleId`(`int64`,`json:",string"` 对齐前端 `IdType`;通用默认角色,非企微专属)
- 配置:系统设置→**常规配置** 加"默认角色"下拉(`general-setting.vue`,复用改造后的 `RoleSelect` 组件支持单选;供企微等自动建号复用)
