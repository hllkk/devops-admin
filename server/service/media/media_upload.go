...
import (
	...
	"github.com/hllkk/devops-admin/server/model/media/request"
	"github.com/hllkk/devops-admin/server/model/media/response"
...
func (s *MediaUploadService) Init(ctx context.Context, userID uint, req request.UploadInitReq) (response.UploadInitResp, error) {
	...
	err := global.OPS_DB.WithContext(ctx).Where("user_id = ? AND file_hash = ? AND status = ?", userID, req.FileHash, media.UploadStatusCompleted).First(&done).Error
	if err == nil && done.MediaID != 0 {
		var m media.FileUploadAndDownload
		if e := global.OPS_DB.WithContext(ctx).First(&m, done.MediaID).Error; e == nil {
			// 为当前用户复制一条媒体库记录,共享 storage key
			nm := media.FileUploadAndDownload{Name: req.FileName, Url: m.Url, Tag: m.Tag, Key: m.Key}
			if e = global.OPS_DB.WithContext(ctx).Create(&nm).Error; e == nil {
				resp.Instant = true
				resp.Media = &nm
				return resp, nil
			}
	...
	err = global.OPS_DB.WithContext(ctx).Where("user_id = ? AND file_hash = ? AND status = ?", userID, req.FileHash, media.UploadStatusUploading).First(&up).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		if err = global.OPS_DB.WithContext(ctx).Create(&up).Error; err != nil {
			return resp, err
		}
	} else if err != nil {
		return resp, err
	...
	// 3) 已收分片索引
	var idx []int
	global.OPS_DB.WithContext(ctx).Model(&media.MediaUploadChunk{}).Where("upload_id = ?", up.ID).
...
func (s *MediaUploadService) SaveChunk(ctx context.Context, userID, uploadID uint, index int, chunkHash string, data []byte) error {
	var up media.MediaUpload
	if err := global.OPS_DB.WithContext(ctx).First(&up, uploadID).Error; err != nil {
...
func (s *MediaUploadService) Complete(ctx context.Context, userID, uploadID uint) (media.FileUploadAndDownload, error) {
	...
	res := global.OPS_DB.WithContext(ctx).Model(&media.MediaUpload{}).
	...
	fail := func(e error) (media.FileUploadAndDownload, error) {
		global.OPS_DB.WithContext(ctx).Model(&media.MediaUpload{}).Where("id = ?", uploadID).Update("status", media.UploadStatusFailed)
		return m, e
	}

	// 校验分片齐全
	var cnt int64
	global.OPS_DB.WithContext(ctx).Model(&media.MediaUploadChunk{}).Where("upload_id = ?", uploadID).Count(&cnt)
	...
	// 回填会话并清理
	global.OPS_DB.WithContext(ctx).Model(&media.MediaUpload{}).Where("id = ?", uploadID).
		Updates(map[string]interface{}{"status": media.UploadStatusCompleted, "media_id": m.ID, "storage_key": key})
	global.OPS_DB.WithContext(ctx).Where("upload_id = ?", uploadID).Delete(&media.MediaUploadChunk{})
...
func (s *MediaUploadService) Cancel(ctx context.Context, userID, uploadID uint) error {
	...
	global.OPS_DB.WithContext(ctx).Where("upload_id = ?", uploadID).Delete(&media.MediaUploadChunk{})
	_ = upload.RemoveUploadDir(uploadID)
	return global.OPS_DB.WithContext(ctx).Delete(&media.MediaUpload{}, uploadID).Error
}
...
func (s *MediaUploadService) CleanupStale(ctx context.Context, ttlHours int) error {
	...
	for _, up := range stale {
		...
		global.OPS_DB.WithContext(ctx).Delete(&media.MediaUpload{}, up.ID)
	}
	return nil
}
