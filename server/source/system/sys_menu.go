...
func (i *initMenu) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	childMenus := []sysModel.SysMenu{
		...
		// ── AI 网关(身份/模型两个顶层目录下的二级管理页;ApiPrefix 沿用后端接口前缀,与菜单 Path 解耦) ──
		{
			...
			Component: "_gateway/gateway/identity/ai-key/index",
		...
		},
		{
			...
			Component: "_gateway/gateway/identity/credential/index",
		...
		},
		{
			...
			Component: "_gateway/gateway/models/provider/index",
		...
		},
		{
			...
			Component: "_gateway/gateway/models/model/index",
			Icon:      "mdi:brain",
			Visible:   "0",
			OrderNum:  2,
			Module:    "gateway",
