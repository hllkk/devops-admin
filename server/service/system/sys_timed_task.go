...
// TimedTaskService 定时任务服务(全局单例 + global.OPS_DB, 遵项目既有约定)
...
func (s *TimedTaskService) checkNameUnique(ctx context.Context, name string, excludeID uint) error {
	var count int64
	db := global.OPS_DB.WithContext(ctx).Model(&system.SysTimedTask{}).Where("name = ?", name)
	if excludeID > 0 {
...
func (s *TimedTaskService) ScheduleTask(t system.SysTimedTask) error {
	...
	var err error
	if t.WithSeconds {
		_, err = global.OPS_Timer.AddTaskByFuncWithSecond(name, t.Spec, fn, t.Name)
	} else {
		_, err = global.OPS_Timer.AddTaskByFunc(name, t.Spec, fn, t.Name)
	}
	return err
}
...
func (s *TimedTaskService) LoadAll(ctx context.Context) error {
	...
	if err := global.OPS_DB.WithContext(ctx).Find(&tasks).Error; err != nil {
		return err
	}
	for i := range tasks {
...
func (s *TimedTaskService) UpdateTimedTask(ctx context.Context, t *system.SysTimedTask) error {
	...
	err := global.OPS_DB.WithContext(ctx).Model(&system.SysTimedTask{}).Where("id = ?", t.ID).
...
func (s *TimedTaskService) DeleteTimedTask(ctx context.Context, id uint) error {
	global.OPS_Timer.Clear(timedTaskCronName(id))
	return global.OPS_DB.WithContext(ctx).Delete(&system.SysTimedTask{}, "id = ?", id).Error
}
...
func (s *TimedTaskService) ToggleTimedTask(ctx context.Context, id uint, enabled bool) error {
	...
	if err := global.OPS_DB.WithContext(ctx).First(&row, id).Error; err != nil {
		return err
	}
	if err := global.OPS_DB.WithContext(ctx).Model(&system.SysTimedTask{}).
...
func (s *TimedTaskService) GetTimedTaskList(ctx context.Context, info systemReq.SysTimedTaskSearch) (list []systemRes.SysTimedTaskRow, total int64, err error) {
	...
	db := global.OPS_DB.WithContext(ctx).Model(&system.SysTimedTask{})
	if info.Name != "" {
	...
	for _, d := range global.OPS_Timer.Snapshot() {
...
func (s *TimedTaskService) GetTimedTaskLogList(ctx context.Context, info systemReq.SysTimedTaskLogSearch) (list []system.SysTimedTaskLog, total int64, err error) {
	limit, offset := info.LimitOffset()
	db := global.OPS_DB.WithContext(ctx).Model(&system.SysTimedTaskLog{})
	if info.TaskId > 0 {
