...
func (*AliyunOSS) UploadFile(ctx context.Context, file *multipart.FileHeader) (string, string, error) {
	...
	yunFileTmpPath := global.OPS_CONFIG.AliyunOSS.BasePath + "/" + "uploads" + "/" + time.Now().Format("2006-01-02") + "/" + file.Filename
	...
	return global.OPS_CONFIG.AliyunOSS.BucketUrl + "/" + yunFileTmpPath, yunFileTmpPath, nil
}
...
func NewBucket() (*oss.Bucket, error) {
	// 创建OSSClient实例。
	client, err := oss.New(global.OPS_CONFIG.AliyunOSS.Endpoint, global.OPS_CONFIG.AliyunOSS.AccessKeyId, global.OPS_CONFIG.AliyunOSS.AccessKeySecret)
	if err != nil {
		return nil, err
	}

	// 获取存储空间。
	bucket, err := client.Bucket(global.OPS_CONFIG.AliyunOSS.BucketName)
	if err != nil {
		return nil, err
	}
