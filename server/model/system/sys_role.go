...
type SysRole struct {
	...
	DefaultRouter     string `json:"defaultRouter" gorm:"default:admin;comment:默认路由(角色登录后默认打开的路由名,如 admin/server/system_user)"` // 默认路由(登录入口;主角色 user.RoleId 的 DefaultRouter 决定)
