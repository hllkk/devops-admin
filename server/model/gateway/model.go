...
// ModelVisibilityUser 模型用户可见性(发布投影表，visibility_type=user 时使用)。
// 与部门投影表同口径：非业务实体，重建时物理删除(同唯一索引理由)。
type ModelVisibilityUser struct {
	global.OPS_MODEL
	ModelId int64 `json:"modelId,string" gorm:"uniqueIndex:idx_gateway_model_visibility_user;comment:关联模型ID"`                   // 关联模型
	UserId  int64 `json:"userId,string" gorm:"uniqueIndex:idx_gateway_model_visibility_user;comment:关联用户ID(sys_users.user_id)"` // 关联用户
}

func (ModelVisibilityUser) TableName() string {
	return "gateway_model_visibility_user"
}
