...
func (m *mongo) Initialization() error {
	...
	if global.OPS_CONFIG.Mongo.IsZap {
		opts = internal.Mongo.GetClientOptions()
	}
	ctx := context.Background()
	config := &qmgo.Config{
		Uri:              global.OPS_CONFIG.Mongo.Uri(),
		Coll:             global.OPS_CONFIG.Mongo.Coll,
		Database:         global.OPS_CONFIG.Mongo.Database,
		MinPoolSize:      &global.OPS_CONFIG.Mongo.MinPoolSize,
		MaxPoolSize:      &global.OPS_CONFIG.Mongo.MaxPoolSize,
		SocketTimeoutMS:  &global.OPS_CONFIG.Mongo.SocketTimeoutMs,
		ConnectTimeoutMS: &global.OPS_CONFIG.Mongo.ConnectTimeoutMs,
	}
	if global.OPS_CONFIG.Mongo.Username != "" && global.OPS_CONFIG.Mongo.Password != "" {
		config.Auth = &qmgo.Credential{
			Username:   global.OPS_CONFIG.Mongo.Username,
			Password:   global.OPS_CONFIG.Mongo.Password,
			AuthSource: global.OPS_CONFIG.Mongo.AuthSource,
		}
	}
...
func (m *mongo) CreateIndexes(ctx context.Context, name string, indexes [][]string) error {
	collection, err := global.OPS_MONGO.Database.Collection(name).CloneCollection()
	if err != nil {
	...
	for k1, v1 := range indexMap1 {
		...
		if len(fmt.Sprintf("%s.%s.$%s", collection.Name(), name, v1)) > 127 {
			err = global.OPS_MONGO.Database.Collection(name).CreateOneIndex(ctx, options.IndexModel{
				Key:          v1,
